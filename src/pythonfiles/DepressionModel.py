...
file_path = 'C:/Users/sachu/Desktop/Project/brfss2013/brfss2013.csv'
...
# %%
"""Data correction for age group less than 30"""
...
columns_to_update_unknown = [
    'seatbelt', 'flushot6', 'tetanus', 'pneuvac3', 'hivtst6',
...
features = ['menthlth', 'poorhlth', 'physhlth', 'X_bmi5', 'drvisits', 'X_llcpwt2', 'X_vegesum', 'fc60_', 'maxvo2_', 'X_impnph']
target_variable = 'addepev2' 
target = df[target_variable]
# Identify numeric and non-numeric columns
numeric_features = df[features].select_dtypes(include=np.number).columns
...
# Reduce the data for faster testing
X_train, X_test, y_train, y_test = train_test_split(features_scaled, target, test_size=0.2, random_state=42)
...
# Create the RandomizedSearchCV object with reduced iterations
random_search = RandomizedSearchCV(model, param_distributions=param_dist, n_iter=5, cv=5, scoring='accuracy', n_jobs=-1, random_state=42)
...
# %%
print(df[['menthlth', 'poorhlth', 'physhlth', 'X_bmi5', 'drvisits', 'X_llcpwt2', 'X_vegesum', 'fc60_', 'maxvo2_', 'X_impnph','addepev2']])

# %%
from joblib import dump

# Save the tuned model to a pickle file
model_filename = 'depression_model.sav'
